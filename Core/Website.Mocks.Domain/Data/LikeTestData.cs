...
namespace Website.Mocks.Domain.Data
{
    public static class LikeTestData
    {
        ...
        {
            Assert.AreEqual<string>(storedLike.AggregateId, retrievedLike.AggregateId);
            Assert.AreEqual<string>(storedLike.BrowserId, retrievedLike.BrowserId);
            Assert.AreEqual<string>(storedLike.LikeContent, retrievedLike.LikeContent);
            Assert.AreApproximatelyEqual<DateTime, TimeSpan>(storedLike.LikeTime, retrievedLike.LikeTime, TimeSpan.FromMilliseconds(1));
